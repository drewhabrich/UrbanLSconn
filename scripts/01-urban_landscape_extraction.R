## HEADER---------------------------
## Script name: urban_landscape_extraction
##
## Purpose of script: to extract extents and land cover from urban areas in Canada
##
...
gpkg_file <- "data/01-NA_gis_data.gpkg"
...
ne_adm1 <- ne_download(scale = 50, category = "cultural",
                       ...
                       returnclass = "sf") %>% 
  filter(iso_a2 == "CA") %>%  ## Change the filter here for different countries
...
can_popc <- st_read("./data/lpc_000b21a_e.shp")
# relational database csv with attributes for each polygon
popctr <- read_csv("./data/POPCTR.csv", col_types = "dc") %>% as_tibble() 
...
### Buffer cities to include boundary land cover types
...
### Write to shapefile for use in other scripts
